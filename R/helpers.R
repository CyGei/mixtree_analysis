...
print.params <- function(x, ...) {
  ...
  cat(" population size:", x$population, "\n")
...
#' generation time distribution, population size, simulation duration, and number of trees.
#'
...
#' @param population Integer. Initial susceptible population size.
...
build_params <- function(off_R, off_k, gt_mu, gt_sd, population, duration, n_trees) {
  offspring_dist <- list(
    name = "nbinom",
...
#'
#' Simulates a single outbreak using specified epidemic parameters until the empirical attack rate
#' matches the expected value implied by the reproduction number.
#'
...
build_tree <- function(params) {
  target_AR <- epitrix::R02AR(params$offspring_dist$parameters$R)

  repeat {
    out <- simulacr::simulate_outbreak(
      duration = params$duration,
      population_size = params$population,
      R_values = params$offspring_dist$r(params$population),
      dist_generation_time = params$generation_time$d(0:params$duration),
      dist_incubation = 1L
    )

    AR <- nrow(out$data) / params$population
    tolerance <- 0.05 # 5% tolerance
    if (abs(AR - target_AR) <= tolerance) break
  }
